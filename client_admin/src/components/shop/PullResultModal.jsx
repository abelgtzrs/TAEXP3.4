...
const PullResultModal = ({ result, onClose }) => {
    ...
    const displayImageUrl = item.imageUrl || item.iconUrlOrKey;
    ...
    const rarityColor = {
        ...
        super_rare: 'text-purple-400', ultra_rare: 'text-yellow-400', epic: 'text-purple-400',
        legendary_snoopy: 'text-yellow-400', common_card: 'text-gray-300', super_card: 'text-purple-400',
        ultra_card: 'text-yellow-400', common_rare: 'text-gray-300'
    };
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
            <div className="widget-container p-8 rounded-lg shadow-2xl text-center max-w-sm w-full mx-4" onClick={(e) => e.stopPropagation()}>
                <h2 className="text-2xl font-bold text-white mb-4">{message}</h2>
                <div className="my-6 p-4 bg-gray-900/50 rounded-lg">
                    {displayImageUrl ? (
                        <img src={displayImageUrl} alt={item.name} className="h-40 mx-auto object-contain"/>
                    ) : (
                ...
                <button onClick={onClose} className="w-full bg-teal-500 hover:bg-teal-600 text-white font-bold py-3 rounded-lg transition">
                    Awesome!
                </button>
            </div>
        </div>
