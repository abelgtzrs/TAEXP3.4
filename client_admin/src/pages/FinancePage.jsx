// --- FILE: client-admin/src/pages/FinancePage.jsx (New File) ---
...
// --- NEW COMPONENT: Category Manager Modal ---
...
const AddTransactionForm = ({ categories, onTransactionAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onTransactionAdded(); // Callback to refresh the list
      // Reset form
      setDescription("");
      setAmount("");
      setCategory("");
...
const FinancePage = () => {
  ...
  const [isCategoryModalOpen, setIsCategoryModalOpen] = useState(false); // New state for modal

  const refreshData = async () => {
    try {
      // No need to set loading to true here for a refresh
      const [transRes, catRes] = await Promise.all([api.get("/finance/transactions"), api.get("/finance/categories")]);
  ...
  return (
    <div>
      <div className="flex justify-between items-start">
        ...
        <StyledButton onClick={() => setIsCategoryModalOpen(true)} className="py-2 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Widget title="Spending Summary">
            <p className="text-text-secondary">Charts will be here.</p>
          </Widget>
        </div>
