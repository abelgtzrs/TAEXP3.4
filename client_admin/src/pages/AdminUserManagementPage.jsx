    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={load}
            className="px-3 py-1 text-xs rounded bg-primary/30 hover:bg-primary/50 flex items-center gap-1"
          >
      ...
      {error && (
        <div className="text-status-danger text-xs bg-status-danger/10 border border-status-danger/30 px-3 py-2 rounded">
          {error}
        </div>
      )}
      {loading ? (
              ...
              {users.map((u) => {
                ...
                return (
                  <tr key={u._id} className="odd:bg-white/0 even:bg-white/5 hover:bg-primary/10 transition">
                    ...
                    <td className="p-2">
                      {editing ? (
                        <input
                          ...
                          className="bg-black/30 border border-white/20 px-2 py-1 rounded w-28"
                        />
                      ) : (
                        u.username
                      )}
                    ...
                    <td className="p-2">
                      {editing ? (
                        <select
                          ...
                          className="bg-black/30 border border-white/20 px-2 py-1 rounded"
                        >
                          {roleOptions.map((r) => (
                    ...
                    <td className="p-2 w-14">
                      {editing ? (
                        <input
                          ...
                          className="bg-black/30 border border-white/20 px-2 py-1 rounded w-14"
                        />
                      ) : (
                        u.level
                      )}
                    ...
                    <td className="p-2 w-20">
                      {editing ? (
                        <input
                          ...
                          className="bg-black/30 border border-white/20 px-2 py-1 rounded w-20"
                        />
                      ) : (
                        u.experience
                      )}
                    ...
                    <td className="p-2">
                      {u.currentLoginStreak}/{u.longestLoginStreak}
                    </td>
                      ...
                      {editing ? (
                        <>
                          <button
                            disabled={saving}
                            onClick={saveEdit}
                            className="inline-flex items-center px-2 py-1 bg-green-600/60 hover:bg-green-600 rounded text-white"
                          >
                            <FiSave size={12} />
                          </button>
                          <button
                            onClick={cancelEdit}
                            className="inline-flex items-center px-2 py-1 bg-red-600/60 hover:bg-red-600 rounded text-white"
                          >
                            <FiX size={12} />
                          </button>
                        </>
                        ...
                        <>
                          <button
                            onClick={() => startEdit(u)}
                            className="inline-flex items-center px-2 py-1 bg-primary/40 hover:bg-primary/60 rounded text-white"
                          >
                            <FiEdit2 size={12} />
                          </button>
                          <button
                            onClick={() => openPw(u._id)}
                            className="inline-flex items-center px-2 py-1 bg-amber-600/50 hover:bg-amber-600 rounded text-white"
                          >
                            <FiKey size={12} />
                          </button>
                        </>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-surface/90 border border-white/10 rounded-lg p-4 w-full max-w-sm space-y-3">
            ...
            <input
              ...
              className="w-full bg-black/40 border border-white/20 px-3 py-2 rounded text-sm"
            />
            <div className="flex justify-end gap-2 text-xs">
              <button onClick={closePw} className="px-3 py-1 rounded bg-white/10 hover:bg-white/20">
                Cancel
              </button>
              <button onClick={submitPw} className="px-3 py-1 rounded bg-primary/50 hover:bg-primary/70 text-white">
                Save
              </button>
            </div>
          </div>
