...
const Terminal = () => {
  ...
  const typeLines = async (linesToType) => {
    ...
    for (const line of linesToType) {
      ...
      setLines((prev) => [
        ...prev,
        { text: "", type: line.type, isTyping: true },
      ]);
      ...
      for (let i = 0; i < line.text.length; i++) {
        ...
        if (Math.random() < GLITCH_CHANCE && line.text[i] !== " ") {
          ...
          setLines((prev) => {
            ...
            nextLines[nextLines.length - 1].text =
              currentText + randomGlitchChar;
            return nextLines;
          });
    ...
    switch (cmd) {
      ...
      case "latest":
        try {
          ...
          await typeLines([
            { text: "Error: Could not fetch latest volume.", type: "error" },
          ]);
        }
        break;
      ...
      case "theme":
        ...
        const matchedTheme = availableThemes.find(
          (theme) => theme.toLowerCase() === inputTheme.toLowerCase()
        );

        if (matchedTheme) {
          setTheme(matchedTheme);
          await typeLines([
            { text: `Theme set to '${matchedTheme}'.`, type: "system" },
          ]);
        } else {
          await typeLines([
            {
      ...
      case "favorite":
        ...
        if (!favNum) {
          await typeLines([
            { text: "Usage: favorite [volume number]", type: "error" },
          ]);
          break;
        }
        try {
      ...
      case "catalogue":
        try {
          ...
          await typeLines([
            { text: "Error: Could not fetch volume catalogue.", type: "error" },
          ]);
        }
        break;
      case "random":
        try {
          ...
          await typeLines([
            { text: "Error: Could not fetch random volume.", type: "error" },
          ]);
        }
        break;
      case "view":
  ...
  return (
    <div
      className={`w-full h-[95vh] p-2 text-[12px]`}
        ...
        {lines.map((line, index) => {
          ...
          return (
            <div key={index} className="flex">
              ...
              {line.isTyping && (
                <span className={`blinking-cursor ${currentTheme.cursor}`}>
                  ▋
                </span>
              )}
            </div>
